...
class LiveMixer(discord.AudioSource):
    ...
    def read(self):
        ...
        audio_chunk = np.clip(final_buffer * current_volume, -32768, 32767).astype(np.int16).tobytes()
        
        # --- LISTENING LOGIC ---
        # In a real environment with a voice-recv extension, you'd capture the sink here.
        # We trigger a background task to emit this audio to the browser every 0.5s
        return audio_chunk
...
async def on_message(message):
    if message.author == bot.user: is_me = True
    elif isinstance(message.channel, discord.DMChannel): is_me = False
    elif message.channel.id == TEXT_ID: is_me = False
    else: return
    ...
    socketio.emit('chat_msg', {
        ...
        'img': img_url,
        'is_me': is_me,
        'author_id': message.author.id # Added for DM capability
    })
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .dm-btn { background: #4f545c; font-size: 10px; padding: 2px 5px; border-radius: 3px; cursor: pointer; margin-left: 5px; vertical-align: middle; }
    ...
    <body>
        ...
        <script>
            ...
            // --- LISTEN LOGIC: Receive audio from server ---
            socket.on('voice_data', d => {
                const blob = new Blob([d.audio], { type: 'audio/wav' });
            ...
            socket.on('chat_msg', d => {
                ...
                const dmBtn = !d.is_me ? `<span class="dm-btn" onclick="openDM('${d.author_id}', '${d.user}')">DM</span>` : '';
                
                let html = `<div class="msg ${isMe}"><img src="${d.pfp}" class="pfp"><div class="msg-content"><b>${d.user}</b> ${dmBtn}<br>${d.text}`;
            ...
            function openDM(uid, name) {
                const msg = prompt(`Send DM to ${name}:`);
                if(msg) socket.emit('send_dm', {uid: uid, text: msg});
            }
...
# --- NEW: VOICE LISTENING LOOP ---
async def listen_loop():
    """Simulates capturing VC audio and sending to browser every 0.5s"""
    while True:
        if vc_client and vc_client.is_connected():
            # In a real scenario with voice-recv, you'd pull from the sink buffer here
            # For now, we emit a heartbeat or 'audio_chunk' if available
            pass 
...
def s_dm(data):
    async def task():
        user = await bot.fetch_user(int(data['uid']))
        if user:
...
def send_history():
    async def task():
        ...
        async for m in chan.history(limit=20):
            h.append({'user': m.author.display_name, 'pfp': str(m.author.display_avatar.url), 'text': m.content, 'img': m.attachments[0].url if m.attachments else None, 'is_me': m.author == bot.user, 'author_id': m.author.id})
