...
import discord
from discord.ext import commands, voice_recv # Ensure discord-ext-voice-recv is installed
...
class WebSink(voice_recv.AudioSink):
    def want_opus(self): return False
    def write(self, user, data):
        if is_listening:
            # Convert PCM 16-bit to Float32 for browser playback
            audio_array = np.frombuffer(data.pcm, dtype=np.int16).astype(np.float32) / 32768.0
            socketio.emit('voice_receive', audio_array.tobytes())
...
async def on_message(message):
    ...
    socketio.emit('chat_msg', {
        'user': message.author.display_name, 'pfp': str(message.author.display_avatar.url),
        'text': message.content, 'img': img_url, 'is_me': message.author == bot.user
    })
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .attach-img { max-width: 100%; border-radius: 4px; margin-top: 8px; }
            ...
            #autocomplete { position: absolute; bottom: 70px; left: 20px; background: #18191c; border-radius: 4px; display: none; width: 200px; z-index: 100; }
            .user-opt { padding: 8px; cursor: pointer; border-bottom: 1px solid #2f3136; }
            #sList { max-height: 150px; overflow-y: auto; }
        </style>
    </head>
    <body>
        <div id="side">
            <div class="section">
                ...
                <button id="listenBtn" class="btn-blue" onclick="toggleListen()">👂 Listen In: OFF</button>
        ...
        <script>
            ...
            function toggleListen() {
                ...
                document.getElementById('listenBtn').innerText = listening ? "👂 Listen In: ON" : "👂 Listen In: OFF";
                if(listening) audioCtx.resume();
            }

            // [Previous JS logic remains identical for Chat, TTS, History, and Uploads]
            ...
            function vAction(a) { socket.emit('voice_action', {action: a, chan: document.getElementById('vSel').value}); }
            ...
            async function uploadSound() {
                ...
                await fetch('/upload_sound', {method: 'POST', body: fd}); loadSounds();
            }
            ...
            async function loadSounds() {
                const res = await fetch('/list_sounds'); const list = await res.json();
                ...
                list.forEach(s => {
                    ...
                    b.onclick = () => socket.emit('play_saved', {n: s}); div.appendChild(b);
                });
            }
            async function checkMention(el) {
                const lastAt = el.value.lastIndexOf('@'); const ac = document.getElementById('autocomplete');
                if(lastAt !== -1) {
                    const res = await fetch(`/search_members?q=${el.value.substring(lastAt+1)}`);
                    const users = await res.json(); ac.innerHTML = '';
                    if(users.length){
...
def v_act(data):
    async def task():
        ...
        if data['action'] == 'join':
            ...
            # Note: We must use VoiceRecvClient for "Listening" to work
            vc_client = await bot.get_channel(VC_CHANNELS[data['chan']]).connect(cls=voice_recv.VoiceRecvClient)
            vc_client.listen(WebSink())
            vc_client.play(mixer)
        else: await vc_client.disconnect()
    asyncio.run_coroutine_threadsafe(task(), bot_loop)

# [Other handlers like upload_sound, send_img, send_chat, etc. remain unchanged from v4]
@app.route('/upload_sound', methods=['POST'])
def up_sound():
    f = request.files['file']; path = os.path.join(UPLOAD_FOLDER, secure_filename(f.filename))
    f.save(path); mixer.add_source(path); return "OK"
@app.route('/send_img', methods=['POST'])
def send_img():
    f = request.files['file']; path = secure_filename(f.filename); f.save(path)
    asyncio.run_coroutine_threadsafe(bot.get_channel(TEXT_ID).send(file=discord.File(path)), bot_loop); return "OK"
...
def search_m():
    q = request.args.get('q', '').lower(); guild = bot.get_channel(TEXT_ID).guild
    return jsonify([{'name': m.display_name} for m in guild.members if q in m.display_name.lower()][:5])
@socketio.on('get_history')
def send_history():
    async def task():
        chan = bot.get_channel(TEXT_ID); h = []
...
def s_chat(data):
    async def task():
        chan = bot.get_channel(TEXT_ID); content = data['text']
...
def s_tts(data):
    p = f"tts_{threading.get_ident()}.mp3"; gTTS(text=data['text'], lang='en').save(p); mixer.add_source(p)
